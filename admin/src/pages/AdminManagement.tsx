...
function AddAdminModal({
  candidates, emails, emailsLoading, onClose, onAdd, saving,
}: {
  candidates: any[];
  ...
  const filtered = useMemo(() => {
    ...
    return candidates.filter((c) =>
      ...
      (emails[c.id] ?? '').toLowerCase().includes(q)
    );
  }, [candidates, search, emails]);
  ...
  const columns: DataTableColumn<any>[] = [
    {
      id: 'name',
      header: 'Name',
      sortValue: (comp) => (comp.name_companion ?? comp.name ?? '').toLowerCase(),
      searchValue: (comp) => comp.name_companion ?? comp.name ?? '',
      cell: (comp) => (
        <span className="font-medium text-slate-900 dark:text-white">
          {comp.name_companion ?? comp.name ?? '—'}
          {comp.id === currentUser?.uid && (
            <span className="ml-2 text-[10px] uppercase text-primary-500 font-semibold">You</span>
          )}
        </span>
      ),
    },
    ...
    },
    {
      id: 'role',
      header: 'Admin Role',
      sortValue: (comp) => toAdminLevel(comp.admin_level),
      cell: (comp) => {
        ...
        return level === 3 ? (
          ...
          <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
            level === 1 ? 'bg-sky-500/15 text-sky-500' : 'bg-emerald-500/15 text-emerald-500'
          }`}>
            {LEVEL_LABEL[level]}
          </span>
        );
  ...
  return (
    <div className="space-y-6 z-10 relative">
      ...
      <DataTable
        ...
        rowKey={(comp) => comp.id}
        loading={loading}
        emptyMessage='Belum ada admin. Klik "Tambah Admin" untuk mengangkat companion.'
