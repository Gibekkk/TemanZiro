...
export default function ScheduleTabs({
  ...
  const currentActiveTab =
    activeTab !== undefined
      ? activeTab
      : internalActiveTab || currentTabs[0] || "Konfirmasi";
  const currentSetActiveTab =
    setActiveTab !== undefined ? setActiveTab : setInternalActiveTab;
    ...
    >
        ...
        {currentTabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              >
                {isFull ? tab.toUpperCase() : tab.toUpperCase()}
              </Text>
            </TouchableOpacity>
          );
        })}
      ...
      <View style={[styles.sectionHeader, isFull && { paddingHorizontal: 20 }]}>
        ...
        {!isFull && (
          <TouchableOpacity style={styles.viewAll} onPress={handleViewAll}>
            <Text
              style={[styles.viewAllText, { color: theme.colors.secondary }]}
            >
              View all
            </Text>
            <IconArrowRight
      ...
      >
        {loading ? (
          ...
          (isFull ? filteredSchedules : filteredSchedules.slice(0, 1)).map(
            (schedule) => (
              <ScheduleCard
                ...
                onPress={() => {
                  router.push("/booker/pendingsession")
                }}
              />
            ),
          )
